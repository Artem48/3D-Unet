...
def dice_coefficient(truth, prediction):
    aa = np.sum(truth * prediction)
    bb = np.sum(truth)
    cc = np.sum(prediction)
    dd = (np.sum(truth) + np.sum(prediction) - np.sum(truth * prediction))
    return np.sum(truth * prediction)/(np.sum(truth) + np.sum(prediction) - np.sum(truth * prediction))
...
def main():
    ...
    plt.ylabel("IoT Coefficient")
    plt.savefig("jaccar_validation_scores_boxplot.png")
    plt.close()
